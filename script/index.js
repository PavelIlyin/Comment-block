...
function showComments() {
    ...
    comments.forEach((item, index) => {
        ...
        commentOption.append(heart);

        let trash = document.createElement('i');
        trash.className = 'gg-trash';
        trash.id = `${index}`;
        trash.addEventListener('click', del);
        commentOption.append(trash);

        commentBox.append(commentBoxName);
        commentBox.append(commentBoxComment);
        commentBox.append(commentBoxDate);
        commentBox.append(commentOption);

        out.push(commentBox);
    })

    commentsBlock.innerHTML = '';
    out.forEach(i => commentsBlock.append(i));
}
